  ...
  const handleUpdateCart = async (e) => {
    ...
    try {
      ...
      for (const item of cartItems) {
        await handleQuantityChange(item.productId, item.quantity);
      }
                    ...
                    cartItems.map((item) => (
                      <tr key={item.productId} className="cart__row border-bottom line1 cart-flex border-top">
                        ...
                        <td className="cart__update-wrapper cart-flex-item text-right">
                          <div className="cart__qty text-center">
                            <div className="qtyField">
                              <a className="qtyBtn minus" href="#" onClick={(e) => { e.preventDefault(); handleQuantityChange(item.productId, item.quantity - 1); }}><i className="icon icon-minus"></i></a>
                              <input className="cart__qty-input qty" type="text" name="quantities[]" value={item.quantity} pattern="[0-9]*" readOnly />
                              <a className="qtyBtn plus" href="#" onClick={(e) => { e.preventDefault(); handleQuantityChange(item.productId, item.quantity + 1); }}><i className="icon icon-plus"></i></a>
                            </div>
                          </div>
                        </td>
                        ...
                        <td className="text-center small--hide">
                          <a href="#" onClick={(e) => { e.preventDefault(); if (window.confirm('Bạn có muốn xóa không?')) handleRemoveItem(item.productId); }} className="btn btn--secondary cart__remove" title="Remove item">
